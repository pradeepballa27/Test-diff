...
import {AiServicesSteps} from "./steps/ai-services";
import {AiServicesStepsSpec} from "./steps/ai-services/constants";
...
function getDefaultStepStartStates(params: {
  ...
  const stepStartStates: StepStartStates = {
    ...
    [StorageStepsSpec.FETCH_STORAGE_FILES.id]: { disabled: false },
    [AiServicesStepsSpec.FETCH_VERTEXAI_MODELS.id]: { disabled: false },
    ...
    [STEP_BIG_QUERY_TABLES]: { disabled: true },
...
async function getStepStartStatesUsingServiceEnablements(params: {
  ...
  const stepStartStates: StepStartStates = {
    ...
    [StorageStepsSpec.FETCH_STORAGE_FILES.id]: createStepStartState(
        ServiceUsageName.FILE,
    ),
    [AiServicesStepsSpec.FETCH_VERTEXAI_MODELS.id]: createStepStartState(
        ServiceUsageName.VERTEXAI,
    ),
  ...
  const apiServiceToStepIdsMap: { [apiService: string]: string[] } = {
    ...
    [ServiceUsageName.FILE]: storageSteps.map((s) => s.id),
    [ServiceUsageName.VERTEXAI]: AiServicesSteps.map((s) => s.id),
