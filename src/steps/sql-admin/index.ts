...
export async function fetchSQLAdminInstances(
  ...
  await client.iterateCloudSQLInstances(async (instance) => {
    ...
    const metrics = await client.getCloudSQLMetrics(context, instance.name ?? "");
    logger.info(
        {
          instance_name: instance.name ?? "",
          metrics: metrics,
        },
        'The discovered CloudSQL metrics',
    );
    if (instance.databaseVersion?.toUpperCase().includes(DATABASE_TYPE.MYSQL)) {
      await jobState.addEntity(createMySQLInstanceEntity(instance, metrics));
    } else if (
      instance.databaseVersion?.toUpperCase().includes(DATABASE_TYPE.POSTGRES)
    ) {
      await jobState.addEntity(createPostgresInstanceEntity(instance, metrics));
    } else if (
      instance.databaseVersion?.toUpperCase().includes(DATABASE_TYPE.SQL_SERVER)
    ) {
      await jobState.addEntity(createSQLServerInstanceEntity(instance, metrics));
    } else {
