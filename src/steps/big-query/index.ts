...
export async function fetchBigQueryDatasets(
  ...
  try {
    await client.iterateBigQueryDatasets(async (dataset) => {
      const datasetId = dataset.datasetReference?.datasetId ?? ''; // datasetId is required

      // fetch metrics
      const metrics = await client.getDatasetMetrics(context, datasetId);

      await jobState.addEntity(createBigQueryDatasetEntity(dataset, metrics));
    });
  } catch (err) {
