...
export class BigQueryClient extends Client {
  ...
  async getDatasetMetrics(
    context: IntegrationStepContext,
    datasetId: string,
  ): Promise<IMetricsResponse> {
    const datasetMetrics: IMetricsResponse = {
      count: 0,
      size: 0,
    };
    ...
    try {
      ...
      logger.info(
        { datasetId: datasetId },
        `Fetching Metrics for Bigquery dataset`,
      );
      const { count, size } = await fetchBigQueryMetricsByProps(
        logger,
        this.client,
        auth,
        this.projectId,
      ...
      datasetMetrics.count = count;
      datasetMetrics.size = size;

      logger.info(
        { datasetMetrics },
        `BigQuery Dataset Metrics for "${datasetId}"`,
      );
      ...
      logger.warn(
        { error },
        `error fetching metrics for big-query dataset "${datasetId}"`,
      );
      return datasetMetrics;
    }
  }
