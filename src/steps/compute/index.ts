...
import {
  ...
  createComputeGlobalAddressEntity, createNetworkLoadBalancerEntity, createRegionNetworkLoadBalancerEntity,
} from './converters';
import {
  ...
  STEP_COMPUTE_IMAGE_KMS_RELATIONSHIPS, ENTITY_TYPE_COMPUTE_NETWORK_LOAD_BALANCER,
} from './constants';
...
import { REGIONS_FIELD, ZONES_FIELD} from "../../constants";
...
export async function fetchComputeSnapshots(
  ...
  let region = getElementFromField(snapshot.sourceDisk?? "", REGIONS_FIELD)
    if (region == "") {
      // sourceDisk can be regional or zonal. (i.e. ...zones/us-central1-a or ...region/us-central1)
      const zone = getElementFromField(snapshot.sourceDisk?? "", ZONES_FIELD)
      region = zone.slice(0,zone.length-2)?? ""
    }
    await jobState.addEntity(createComputeSnapshotEntity(snapshot, region));
  });
}
...
export async function fetchComputeRegionLoadBalancers(
  ...
  await client.iterateRegionLoadBalancers(async (loadBalancer) => {
    const region = getElementFromField(loadBalancer.region?? "", REGIONS_FIELD)
    const loadBalancerEntity = createRegionLoadBalancerEntity(loadBalancer, region);
...
export async function fetchComputeRegionBackendServices(
  ...
  await client.iterateRegionBackendServices(async (regionBackendService) => {
    let regionBackendServiceEntity;
    const region = getElementFromField(regionBackendService.region?? "", REGIONS_FIELD)
    // there are two types of load balancers, application and network. Application lb comes under url_map has backend service attached
    // However, network lb resource comes under backend service, therefore, we can separate these two with their protocols
    if (regionBackendService.protocol != "HTTP" && regionBackendService.protocol != "HTTPS" && regionBackendService.protocol != "HTTP2") {
      regionBackendServiceEntity = createRegionNetworkLoadBalancerEntity(regionBackendService, region);
    } else {
      regionBackendServiceEntity = createRegionBackendServiceEntity(regionBackendService, region);
    }
...
export async function fetchComputeBackendServices(
  ...
  await client.iterateBackendServices(async (backendService) => {
    let backendServiceEntity;
    // see comments under fetchComputeRegionBackendServices
    if (backendService.protocol != "HTTP" && backendService.protocol != "HTTPS" && backendService.protocol != "HTTP2") {
      const region = getElementFromField(backendService.region?? "", REGIONS_FIELD)
      backendServiceEntity = createNetworkLoadBalancerEntity(backendService, region);
    }
    else {
      backendServiceEntity = createBackendServiceEntity(backendService);
    }
...
export const computeSteps: GoogleCloudIntegrationStep[] = [
  ...
  },
  {
    ...
    entities: [
      ...
      {
        resourceName: 'Compute Network LB Backend Service',
        _type: ENTITY_TYPE_COMPUTE_NETWORK_LOAD_BALANCER,
        _class: ENTITY_CLASS_COMPUTE_BACKEND_SERVICE,
      },
    ],
    relationships: [
