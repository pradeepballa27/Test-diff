import { file_v1, storage_v1 } from 'googleapis';
import { parseTimePropertyValue } from '@jupiterone/integration-sdk-core';
import { StorageEntitiesSpec } from './constants';
import { createGoogleCloudIntegrationEntity } from '../../utils/entity';
import { isMemberPublic } from '../../utils/iam';
import { IMetricsResponse } from '../../utils/metrics';
  ...
  return createGoogleCloudIntegrationEntity(data, {
    entityData: {
      ...
      assign: {
        ...
        metricsObjectCount: metrics?.count ?? 0,
        metricsBucketSizeBytes: metrics?.size ?? 0,
      },
    },
  });
}

export function createCloudStorageFileEntity({
  data,
  region,
  projectId,
}: {
  data: file_v1.Schema$Instance;
  region: string;
  projectId: string;
}) {
  return createGoogleCloudIntegrationEntity(data, {
    entityData: {
      source: data,
      assign: {
        _class: StorageEntitiesSpec.STORAGE_FILE._class,
        _type: StorageEntitiesSpec.STORAGE_FILE._type,
        _key: data.name as string,
        id: `file:${data.name}` as string,
        name: data.name,
        displayName: data.name as string,
        description: data.description,
        createdOn: parseTimePropertyValue(data.createTime),
        classification: null,
        // Filestore instances are encrypted by default
        encrypted: true,
